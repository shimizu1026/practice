...
const savePokemonData = (data) => {
  try {
    ...
    console.error("ポケモンデータの保存に失敗:", error);
    return false;
  }
};
...
const loadPokemonData = () => {
  try {
    ...
    console.error("ポケモンデータの読み込みに失敗:", error);
    return defaultPokemonData;
  }
};
...
const loadFilterState = () => {
  try {
    ...
    if (savedState) {
      ...
      typeSelectElem.value = filterState.type || "すべて";
      sortSelectElem.value = filterState.sort || "id-asc";
    }
  } catch (error) {
